/*:
 * @target MZ
 * @plugindesc [MultiTweaks] [v1.7.2] [General, map, zoom, battle, audio, options...]
 * @author ScSWinter
 *
 * @help
 * =======================================================
 ...
 * please consider add a big margin in order to avoid black.
 *
 ...
 * one third, etc, 999999 to disable (default 999999).
 * @default 999999
 *
 ...
 * @min 1
 * @max 3
 * @desc Auto-tiles tilemap animation speed (default 1).
 * @default 1
 *
...
(function() {
  ...
  }catch(e){ ScSWinter.RemoveOptions=[]; }
  ...
  if(ScSWinter.OptimActive){
    ...
    if(ScSWinter.OptimOption.length>0){
      ...
      Object.defineProperty(ConfigManager,'optimGame',{
        get:function(){ return ConfigManager._optimGame; },
        set:function(value){ ConfigManager._optimGame=value; },
        configurable:true
      });
    }
    Graphics._skipNorm=parseInt(ScSWinter.params['General frame skip'])||999999;
    ...
    Graphics._onTick = function(deltaTime){
      ...
      if(Graphics._isStageX){
        ...
        Graphics._throttle = (Graphics._throttle+1)%(Graphics._isEffect?Graphics._skipEffe:(Graphics._isBattle?Graphics._skipBatt:Graphics._skipNorm));
      }
      if(this._canRender()){
        this._app.render();
      }
  ...
  if(ScSWinter.EnemiesClass){
    ...
    Game_Enemy.prototype.s_calcExpQuant = function(){
      if(ScSWinter.EnemyExpGained=="Next-level class value" && this.s_roleid>0){
        ...
        this.s_exp=s_fExp(this.s_level+1,bexp[0],bexp[1],bexp[2],bexp[3])-s_fExp(this.s_level,bexp[0],bexp[1],bexp[2],bexp[3]);
    ...
    Game_Enemy.prototype.s_calcGoldQuant = function(){
      if(ScSWinter.EnemyGoldGained=="Next-level class value" && this.s_roleid>0){
        var bexp = $dataClasses[this.s_roleid].expParams;
        this.s_gold=s_fExp(this.s_level+1,bexp[0],bexp[1],bexp[2],bexp[3])-s_fExp(this.s_level,bexp[0],bexp[1],bexp[2],bexp[3]);
    ...
    if(object){
      ...
      if(object===$dataMap){
        ...
        if(object.titleBgm && object.titleBgm.name){
          Utils.setEncryptionInfo(!!object.hasEncryptedImages, !!object.hasEncryptedAudio, object.encryptionKey);
...
(function() {
  ...
  Game_Map.prototype.scrollLeft = function(distance) {
      ...
      } else if(this._currentZoom>1.0 || ScSWinter.FreeCamera){
