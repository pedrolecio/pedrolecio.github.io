 ...
 * @target MV MZ
 *
 * @plugindesc [1.6.1] Create a Book Menu
...
LibraryData.prototype.loadBooksFromFile = function() {
    ...
    this.loadJSON(function(response){
        ...
        for(var k in $dataBooks._books){
            ...
            if(!$dataBooks._books[k][$dataBooks._bookVisibleTextObject] || $dataBooks._visibleBookTexts[k][$dataBooks._bookVisibleTextObject].length <= 0){
                $dataBooks._visibleBookTexts[k] = [];
            ...
            var inlineImages = $dataBooks._books[k][$dataBooks._bookTextObject].match(/\%(?:in_)?img\(\s*["']([^"']+)["']/gm);
            if(inlineImages && inlineImages.length > 0){
                ...
                $dataBooks._inlineImageIndex[k] = Object.keys(tmp);
            }
...
LibraryData.prototype.loadBooksFromParameters = function(){
    ...
    for(var i=0; i < books.length; i++){
        ...
        if(this._books[key] !== undefined){
            ...
            var inlineImages = b.Text.match(/\%(?:in_)?img\(\s*["']([^"']+)["']/gm);
            if(inlineImages && inlineImages.length > 0){
                var tmp = {};
                inlineImages.forEach(function(i){
                    i.match(/\%(?:in_)?img\(\s*["']([^"']+)["']/m);
                ...
                this._inlineImageIndex[key] = Object.keys(tmp);
            }
            ...
            case 'Icon':
                ...
                switch(iconSettings['Position']){
                    case 'Left':
                        ...
                        break;
                    case 'Center':
                        x = Math.floor((this.windowWidth() - Window_Base._iconWidth) / 2);
                        break;
                    case 'Right':
                        x = this.windowWidth() - 2*this.standardPadding() - Window_Base._iconWidth;
                        break;
                }
